...
#
# Env vars matching HOSTRUN_FORWARD (default: names starting with SBC_) are
# forwarded to the host command. Run it in the background (it blocks for the whole
# deploy) and watch the streamed log.
set -uo pipefail
...
# Build the request JSON, write atomically. Forwards env vars whose names match the
# HOSTRUN_FORWARD prefix (default SBC_) plus an optional HOSTRUN_TIMEOUT.
HOSTRUN_FORWARD="${HOSTRUN_FORWARD:-SBC_}" python3 - "$id" "$@" >"$BOX/request.json.tmp" <<'PY'
import json, os, sys
rid, argv = sys.argv[1], sys.argv[2:]
prefix = os.environ.get("HOSTRUN_FORWARD", "SBC_")
